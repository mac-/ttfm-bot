...
module.exports = function StatsTrackerAddOn(bot, dbConnString) {
	...
	achievementsTracker.on('new', function(userId, ach) {
		bot.getUserInfo(userId, function(userData) {
			bot.speak('Congrats @' + userData.name + '! You got some new achievements: ' + ach.join(', '));
		});
	});
