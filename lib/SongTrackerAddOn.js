...
module.exports = function SongTrackerAddOn(bot) {
	...
	var snags = 0,
		...
		announceStats = function(msgData, issuerId, replyFunc) {
			...
			var title = args.join(' ');
			songTracker.getAllSongDataByTitle(title, function(err, songData) {
				if (err || !songData) {
				...
				if (songData.length > 1) {
					...
					announceSongData(songData[0], replyFunc);
				}
			});
		},
			...
			if (data.success) {
				...
				setTimeout(function() {
					...
					songTracker.getSongData(data.room.metadata.current_song._id, function(err, songData) {
						...
						announceSongData(songData, function(text, callback) { bot.multiSpeak(text, 300, callback); });
					});
				}, 2000);
			}
		},
		announceSongData = function(songData, replyFunc) {

			var numPlays = songData.startTimes.length,
				lastPlayTime = moment(songData.startTimes[songData.startTimes.length-1]).fromNow(),
	...
	this.commands = [{
		...
		},
		{
			...
			secondaryCommands: [],
			help: 'show stats of a song by song title',
			moderatorOnly: false,
