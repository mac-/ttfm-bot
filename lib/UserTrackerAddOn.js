...
module.exports = function UserTrackerAddOn(bot, options) {
	options = options || {};

	var UserTracker = require('./domain/UserTracker'),
		userTracker = new UserTracker(options.dbConnectionString),
		...
		unbanUser = function(msgData) {
			...
			userTracker.checkUserByName(name, function(user) {
				if (user[0]) {
		...
		connectHandler = function() {
			bot.getCurrentUsersInRoom(function(users) {
				...
				for (var prop in users) {
					userTracker.checkUser(prop, function(user) {
