...
function ExtendedBot(auth, userId, roomId) {
				...
				action: function(msgData) {
					...
					if (!addOnName) {
						...
						if (optionValue) {
							...
							if (Type === Boolean && (optionValue === 'true' || optionValue === 'false')) {
		...
		commands = [
			{
				...
				moderatorOnly: false,
				action: function(msgData, issuerId, replyFunc) {
			...
			},
			{
				primaryCommand: '/kick',
				secondaryCommands: [],
				help: 'kick a user from the room',
				moderatorOnly: true,
				action: function(msgData, issuerId, replyFunc) {
					var args = msgData.text.split(/\s/);
					args.shift();
					var user = args.join(' ');
					self.findUserIdInRoomByName(user, function(userId) {
						if (!userId) {
							return replyFunc('Unable to find user: ' + user);
						}
						var reason = kickReasons[Math.round(Math.random() * (kickReasons.length - 1))];
						self.boot(userId, reason);
					});
			...
			},
			{
				primaryCommand: '/rmdj',
				secondaryCommands: [],
				help: 'removes a user from the stage',
				moderatorOnly: true,
				action: function(msgData, issuerId, replyFunc) {
					var args = msgData.text.split(/\s/);
					args.shift();
					if (args.length < 1) {
						return replyFunc('Please provide a user name!');
					}
					var user = args.join(' ');
					self.findUserIdInRoomByName(user, function(userId) {
						if (!userId) {
	...
	*/
	this.on('registered', function(data) {
		if (data.success && userId === data.user[0].userid) {
			...
			/*
			// wonkiness with ttapi and having fans
			self.getFanOf(function(fanData) {
				if (fanData.success) {
					_.each(fanData.fanof, function(fanId) {
						self.removeFan(fanId);
					});
				}
			});
			*/
			...
			self.roomInfo(function(roomData) {
				if (roomData.success) {
					...
					if (roomData.room.metadata.current_song) {
						currentSong = { _id: roomData.room.metadata.current_song._id, starttime: roomData.room.metadata.current_song.starttime };
						_.extend(currentSong, roomData.room.metadata.current_song.metadata);
						_.extend(currentSong, { current_dj: roomData.room.metadata.current_dj });
					}
						...
						_.each(roomData.djids, function(djid) {
							self.getUserInfo(djid, function(userData) {
								if (userData) {
									...
									console.log('Unable to get user data, and therefore unable to emit connect event');
								}
							});
						});
					}
	...
	*/
	...
	this.on('newsong', function(data) {
		if (data.success) {
			currentSong = { _id: data.room.metadata.current_song._id, starttime: data.room.metadata.current_song.starttime };
			_.extend(currentSong, data.room.metadata.current_song.metadata);
			_.extend(currentSong, { current_dj: data.room.metadata.current_dj });
		}
	});
		...
		if (data.success) {
			this.getUserInfo(data.user[0].userid, function(userData) {
				if (userData) {
					...
					console.log('Unable to track the DJ that was added:', data.user[0].userid);
				}
			});
		}
	});
	...
	*/
	...
	/*
		Gets the info of a user in the room by user ID.
	...
	*/
	this.getUserInfo = function(uId, callback) {
		var userInfo = cache.get('user:' + uId);
		if (!userInfo) {
			self.getProfile(uId, function(userData) {
				if (userData) {
					cache.put('user:' + uId, userData, cacheDuration, true);
					callback(userData);
				}
				else {
					console.log('Unable to get userInfo for:', uId);
					callback(null);
				}
			});
		}
