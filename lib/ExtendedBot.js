...
function ExtendedBot(auth, userId, roomId) {
	...
	*/
	this.on('registered', function(data) {
		if (data.success && userId === data.user[0].userid) {
			...
			cache.put('user:' + data.user[0].userid, userData, cacheDuration, true);
	...
	this.on('update_user', function(data) {
		...
		cache.put('user:' + data.userid, userData, cacheDuration, true);
	});
	...
	*/
	this.getUserInfo = function(userId, callback) {
		...
		if (!userInfo) {
			this.stalk(userId, true, function(roomData) {
				if (userData.success) {
	...
	*/
	this.getModsInRoom = function(callback) {
		...
		if (!modInfo) {
			this.roomInfo(function(roomData) {
				console.log(roomData.room.metadata.moderator_id);
				cache.put(cacheKey, roomData.room.metadata.moderator_id, roomCacheDuration, false);
				callback(roomData.room.metadata.moderator_id);
			});
		}
		else {
			console.log('cached', modInfo);
			callback(modInfo);
		}
	};
