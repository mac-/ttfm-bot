...
function ExtendedBot(auth, userId, roomId) {
	...
	*/
	this.on('registered', function(data) {
		...
		// greet all moderators
		else if (data.success) {
			self.roomInfo(function(roomData) {
				if (roomData.room.metadata.moderator_id.indexOf(data.user[0].userid) >= 0) {
	...
	*/
	...
	this.on('pmmed', function(pmData) {
		...
		if (self.userId !== pmData.senderid) {
			self.roomInfo(function(roomData) {
				var isMod = (roomData.room.metadata.moderator_id.indexOf(pmData.senderid) >= 0);
				
				action = findActionByCommand(pmData.text, isMod);
				// if action was found, invoke it
				action && action(pmData);
			});
		}
	});
	...
	this.on('speak', function(msgData) {
		var action = null;
		if (self.userId !== msgData.senderid) {
			action = findActionByCommand(msgData.text);
			// if action was found, invoke it
			action && action(msgData);
		}
	});
	...
	/*
		...
		describes the add on and "enable" and "disable" methods which should enable and disable the
		...
		Params:
			...
			options		{Object}	A object containing options that will be passed to the add on.
		Returns:
			None
	*/
	...
	*/
	this.getUserInfo = function(userId, callback) {
		var userInfo = cache.get(userId);
		if (!userInfo) {
			this.stalk(userId, true, function(userData) {
				if (userData.success) {
					self.removeFan(userId, function() {
						var cacheDuration = 60 * 60 * 4; // cache user info for 4 hours
						cache.put(userId, userData, 60, true);
	...
	/*
		...
		Params:
			callback	{Function}	The function to call when the song is determined. The function
									should expect to receive one parameter which will be an object
									with all the properties of the song including: _id, album, artist,
									coverart, song, mnid, and length... or null if there is no current song
		Returns:
			None
	*/
	this.getCurrentSong = function(callback) {
		this.roomInfo(true, function(roomData) {
			if (roomData.success && roomData.room.metadata.current_song) {
				var result = { _id: roomData.room.metadata.current_song._id };
				_.extend(result, roomData.room.metadata.current_song.metadata);
				callback(result);
			}
			else {
				callback(null);
