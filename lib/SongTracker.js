...
var SongTracker = function(dbConnectionString) {
	...
	this.getSongsWithMostUpVotes = function(limit, callback) {
		limit = (limit > 10) ? 10 : Math.max(1, limit);
		var query = Song.find({upVotes: {$gte: 2}});

		query.limit(limit).sort('-upVotes').select('song artist upVotes').exec(function(err, doc) {
			if (doc) {
				callback(err, doc);
			}
			else {
