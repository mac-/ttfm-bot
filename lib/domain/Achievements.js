...
module.exports = [
	...
	/*{
		...
		description: 'play a song while AFK',
		icon: ':zzz:',
		check: function(oldData, newData, userId, bot) {
			if (!newData.downVotesRecieved) return false;
			return (newData.downVotesRecieved >= 50);
	...
	{
		name: '',
		description: 'vote for the same song more than once',
		icon: '',
		check: function(oldData, newData, userId, bot) {
			if (!newData.upVoteLog) return false;
			return (newData.upVoteLog[0].userId === userId);
		}
	},*/
...

	{
		...
		icon: ':hocho:',
		check: function(oldData, newData, userId, bot) {
			if (!newData.upVoteLog || newData.upVoteLog.length < 1) return false;
			return (newData.upVoteLog[0].userId === userId);
		}
	},
	{
		...
		check: function(oldData, newData, userId, bot) {
			if (!newData.upVoteLog || newData.upVoteLog.length < 2) return false;
			return (newData.upVoteLog[1].userId === userId);
		}
	},
...

	{
		...
		icon: ':tongue:',
		check: function(oldData, newData, userId, bot) {
			if (!newData.upVotes || !newData.djIds) return false;
			return (newData.djIds[0] === userId && newData.upVotes >= 5);
		}
	},
	{
		name: 'It\'s So Saxy',
		...
		check: function(oldData, newData, userId, bot) {
			if (!newData.upVotes || !newData.djIds) return false;
			return (newData.djIds[0] === userId && newData.upVotes >= 10);
		}
	},
	{
		name: 'I Know What\'s Best',
		description: 'be the first DJ of a song that eventually gets 25 up votes',
		icon: ':crystal_ball:',
		check: function(oldData, newData, userId, bot) {
			if (!newData.upVotes || !newData.djIds) return false;
			return (newData.djIds[0] === userId && newData.upVotes >= 25);
		}
	},
	{
		...
		check: function(oldData, newData, userId, bot) {
			if (!newData.upVotes || !newData.djIds) return false;
			return (newData.djIds[0] === userId && newData.upVotes >= 50);
		}
	},
	{
		name: 'God Is On My Side',
		description: 'be the first DJ of a song that eventually gets 75 up votes',
		icon: ':pray:',
		check: function(oldData, newData, userId, bot) {
			if (!newData.upVotes || !newData.djIds) return false;
			return (newData.djIds[0] === userId && newData.upVotes >= 75);
		}
	}
