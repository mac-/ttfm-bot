		...
		action: function(bot, msgData) {
			...
			bot.once('song_stats', function(data) {
				...
				var numPlays = data.startTimes.length,
					timezoneOffset = (new Date().getTimezoneOffset() === 0) ? 420 * 60 * 1000 : 0,
					prevPlayTime = new Date(data.startTimes[data.startTimes.length-2] - timezoneOffset),
					lastPlayTime = (data.startTimes.length <= 1) ? 'Never' : moment(prevPlayTime).calendar(),
				...
				bot.stalk(djWithMostPlays, true, function(userData) {
					...
					msgArray.push('Last Played On: ' + lastPlayTime);
