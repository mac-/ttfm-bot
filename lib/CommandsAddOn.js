...
var commands = [
	{
		...
		action: function(bot, msgData) {
			...
			for (var i = 0; i < commands.length; i++) {
				(function(index) {
					setTimeout(function() { bot.pm(':point_right: ' + commands[index].primaryCommand + ' - ' + commands[index].help, msgData.userid); }, (index+1) * 300);
				}(i));
			}
		}
	},
		...
		action: function(bot, msgData) {
			...
			bot.once('top_songs', function(data) {
				for (var i = 0; i < data.length; i++) {
					(function(index) {
						setTimeout(function() {
							bot.speak('"' + data[index].song + '" By ' + data[index].artist + ' (' + data[index].upVotes + ' up votes)');
						}, 300 * index);
					}(i));
				}
		...
		action: function(bot, msgData) {
			...
			bot.once('bottom_songs', function(data) {
				for (var i = 0; i < data.length; i++) {
					(function(index) {
						setTimeout(function() {
							bot.speak('"' + data[index].song + '" By ' + data[index].artist + ' (' + data[index].downVotes + ' down votes)');
						}, 300 * index);
					}(i));
				}
