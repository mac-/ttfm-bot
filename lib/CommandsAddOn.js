...
var commands = [
	...
	},
	{
		...
		secondaryCommands: [],
		help: 'get escorted off stage after the current song ends',
		action: function(bot, msgData) {
			bot.stalk(msgData.userid, true, function(data) {
				bot.removeFan(msgData.userid);
				if (msgData.userid === data.user.userid) {
					bot.speak('Ok, I will escort @' + msgData.name + ' off the stage after this song.');
					bot.once('endsong', function(songData) {
						bot.remDj(msgData.userid);
					});
				}
			});
