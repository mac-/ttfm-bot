...
var commands = [
	{
		...
		action: function(bot, msgData) {
			...
			setTimeout(function() { bot.pm('Available Commands:', msgData.userid) }, 300);
	...
	},
	{
		...
		secondaryCommands: ['^.*hi tes.*$', '^.*hello tes.*$', '^.*whats? ?up tes.*$'],
		help: 'say "hi" to me',
		action: function(bot, msgData) {
			...
			if (bot.addOns && bot.addOns.statsTracker && bot.addOns.statsTracker.getAchievementsByUserId) {
				...
				bot.addOns.statsTracker.getAchievementsByUserId(msgData.userid, function(err, achievements) {
					...
					if (achievements.length > 0) {
						bot.speak('Achievements for @' + msgData.name + ': ' + achievements.join(', '));
					}
					else {
